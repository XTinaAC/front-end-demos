...
fn main() {
    ...
    loop {
        ...
        let number_of_bytes = io::stdin()
            .read_line(&mut guess)
            .expect("Failed to read a line");

        println!(
            "You guessed: {}(Number of bytes: {})",
            guess, number_of_bytes
        );
        ...
        // A [match] expression is made up of [arms] which consist of a [pattern] to match against,
